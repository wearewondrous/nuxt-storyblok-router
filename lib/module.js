...
export default async function storyblokRouterModule (moduleOptions) {
  ...
  pages.forEach((story) => {
    ...
    let fullSlug = stripTrailingSlash(story.full_slug)
    ...
    languageCodes.forEach((lang) => {
      if (Array.isArray(story.translated_slugs) && story.translated_slugs.length > 0) {
        const translatedSlug = story.translated_slugs.find(slug => slug.lang === lang)
        fullSlug = translatedSlug.path || fullSlug
      }
