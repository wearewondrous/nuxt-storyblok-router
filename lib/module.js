...
export default async function storyblokRouterModule (moduleOptions) {
  ...
  const routes = pages.map((story) => {
    ...
    const slug = stripTrailingSlash(`${fullSlug === 'home' ? '' : '/' + fullSlug}`)
    const path = `${langPrefix}${slug}`

    const alias = languageCodes.map((lang) => {
      if (
        Array.isArray(story.translated_slugs) &&
        story.translated_slugs.length > 0
      ) {
        const translatedSlug = story.translated_slugs.find(
          slug => slug.lang === lang
        )
        return `/${translatedSlug.path}` || slug
      }
      return slug
    })
